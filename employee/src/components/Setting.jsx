...
export default function Setting() {
  ...
  const fetchProfile = async () => {
    try {
      ...
      const { data } = await axios.get("http://localhost:3000/employee/profile", {
  ...
  const handleSave = async () => {
    ...
    try {
      const token = sessionStorage.getItem('employeeToken');
      await axios.put("http://localhost:3000/employee/profile", formData, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handlePasswordChange = async () => {
    ...
    try {
      ...
      const token = sessionStorage.getItem('employeeToken');
      await axios.put("http://localhost:3000/employee/change-password", {
